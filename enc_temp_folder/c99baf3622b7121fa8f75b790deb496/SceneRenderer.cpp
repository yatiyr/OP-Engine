	...
	// ---------------------------- UTILITY FUNCTIONS ------------------------------------ //
		...
		// ----------- CASCADED SHADOW MAPPING ------------ //
			...
			static glm::mat4 GetLightLightSpaceMatrix(const glm::mat4& projectionMatrix, const glm::mat4& viewMatrix, const float nearPlane, const float farPlane, const glm::vec3& lightDir, const float zMult)
			{
				...
				const glm::mat4 newProjectionMatrix = glm::perspective(fovy, aspectRatio, nearPlane, farPlane);

				const std::vector<glm::vec4> frustumCornerCoordinates = GetFrustumCornerCoordinatesWorldSpace(newProjectionMatrix * viewMatrix);
				                                         ...
				                                         const glm::vec3& lightDir, const float zMult)
			{
				...
				for (uint32_t i = 0; i < cascadeLevels.size() + 1; i++)
				{
					if (i == 0)
					{
