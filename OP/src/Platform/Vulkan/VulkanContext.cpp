...
namespace OP
{
	...
	// ***************************** SWAPCHAIN RENDERPASS AND FRAMEBUFFERS ************************************* //
	void VulkanContext::CreateSwapchainRenderPass()
	{
		...
		tSpec.TextureFormat = AttachmentFormat::BGRA8;
		tSpec.Samples       = AttachmentSample::SAMP1;
		tSpec.ResAttachment = ResolveAttachment::None;

		spec.Attachments = { tSpec };
		m_SwapChainRenderPass = std::make_shared<VulkanRenderPass>(spec);
		*/

		// TODO: Take this out of vulkan context
		AttachmentSpecification spec;
		...
		tSpecDepth.Samples       = AttachmentSample::SAMP8;
	...
	void VulkanContext::CreateSwapchainFramebuffers()
	{
		...
		// Clear and resize framebuffers of the swapchain
		m_SwapChainFramebuffers.clear();
		m_SwapChainFramebuffers.resize(m_SwapChainImages.size());

		// Create framebuffers with the size of the swapchain extent
