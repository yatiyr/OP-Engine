...
// application or OP engine, we define OPIUM_API being
// import or export
#ifdef OP_PLATFORM_WINDOWS
	#if OP_DYNAMIC_LINK
		#ifdef OP_BUILD_DLL
			#ifdef __GNUC__
				#define OP_API __attribute__ ((dllexport))
			#else
				#define OP_API __declspec(dllexport)
			#endif
		#else
			#ifdef __GNUC__
				#define OP_API __attribute__ ((dllimport))
			#else
	...
	#if OP_DYNAMIC_LINK
		#if __GNUC__ >= 4
			#define OP_API __attribute__ ((visibility ("default")))
