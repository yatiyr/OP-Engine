...
namespace YAML
{
	...
	struct convert<glm::mat4>
	{
		...
		{
			Node node;
			node.push_back(mat[0].x);
			node.push_back(mat[0].y);
			node.push_back(mat[0].z);
			node.push_back(mat[0].w);

			node.push_back(mat[1].x);
			node.push_back(mat[1].y);
			node.push_back(mat[1].z);
			node.push_back(mat[1].w);

			node.push_back(mat[2].x);
			node.push_back(mat[2].y);
			node.push_back(mat[2].z);
			node.push_back(mat[2].w);

			node.push_back(mat[3].x);
			node.push_back(mat[3].y);
			node.push_back(mat[3].z);
			node.push_back(mat[3].w);
...
namespace OP
{
	...
	static void SerializeEntity(YAML::Emitter& out, Entity entity)
	{
		...
		if (entity.HasComponent<TransformComponent>())
		{
			...
			out << YAML::Key << "LocalTransform" << YAML::Value << tc.localTransformation;
		...
		if (entity.HasComponent<Physics3DMaterial>())
		{
			...
			out << YAML::Key << "IsKinematic" << physics3DMaterial.IsKinematic;
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				if (transformComponent)
				{
					...
					tc.localTransformation = transformComponent["LocalTransform"].as<glm::mat4>();
				}
